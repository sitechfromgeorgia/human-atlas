...
import {translate,anatomyName} from './i18n';
...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});}catch{onError(translate('errors.webglUnavailable'));return;}
  ...
  renderer.domElement.setAttribute('aria-label',translate('a11y.sceneCanvas'));
  ...
  const loadChunk=async(ci:number)=>{
   ...
   groups.forEach((gs,system)=>{const geometry=mergeGeometries(gs,false);if(!geometry)throw new Error(translate('errors.geometryAssemblyFailed'));geometries.push(geometry);const mesh=new T.Mesh(geometry,mats.get(system as never));mesh.frustumCulled=false;scene.add(mesh);});
   lastState=null;loaded++;onProgress(Math.round(loaded/atlas.chunks.length*100));dirty=true;
  };
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)onError(e instanceof Error?e.message:translate('errors.anatomyLoadFailed'));}})();
  ...
  const contextLost=(e:Event)=>{e.preventDefault();onError(translate('errors.contextLost'));};renderer.domElement.addEventListener('webglcontextlost',contextLost);
